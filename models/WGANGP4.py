...
class WGANGP():
    def __init__(self
        ...
        self.learning_rate = ExponentialDecay(initial_learning_rate,decay_steps=5500,decay_rate=0.98)
    ...
    def get_opti(self):
        if self.optimiser == 'adam':
            opti = Adam(learning_rate=self.learning_rate, beta_1=0.5)
    ...
    def _build_adversarial(self):
        ...
        self.model.compile(optimizer=self.get_opti(),loss=self.wasserstein)
    ...
    , using_generator = False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            print ("%d (%d, %d) [D loss: (%.4f)(R %.4f, F %.4f, G %.4f)] [G loss: %.1f]" % (epoch, critic_loops, 1, d_loss[0], d_loss[1],d_loss[2],d_loss[3],g_loss))
    ...
    def sample_images(self, run_folder):
        ...
        for i in range(r):
            for j in range(c):
                axs[i,j].imshow(np.squeeze(gen_imgs[cnt, :,:,:]), cmap = plt.cm.jet)
                axs[i,j].axis('off')
                cnt += 1
