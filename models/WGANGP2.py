...
class WGANGP():
    def __init__(self
        ...
        , generator_compress_filters
        , generator_compress_kernel_size
        , generator_compress_strides
        ...
        self.generator_compress_filters = generator_compress_filters
        self.generator_compress_kernel_size = generator_compress_kernel_size
        self.generator_compress_strides = generator_compress_strides
    ...
    def _build_generator(self):
        ...
        for i in range(self.n_layers_generator):
            ...
            if self.generator_upsample[i] == 2:
                ...
                x = Conv2D(
                filters = self.generator_compress_filters[i]
                , kernel_size = self.generator_compress_kernel_size[i]
                , padding = 'same'
                , strides = self.generator_compress_strides[i]
                , use_bias=True
                , name = 'compress_' + str(i)
                , kernel_initializer = self.weight_init
                )(x)
    ...
    , using_generator = False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            print ("%d (%d, %d) [D loss: (%.4f)(R %.4f, F %.4f, G %.4f)] [G loss: %.4f]" % (epoch, critic_loops, 1, d_loss[0], d_loss[1],d_loss[2],d_loss[3],g_loss))
    ...
    def sample_images(self, run_folder):
        ...
        for i in range(r):
            for j in range(c):
                axs[i,j].imshow(np.squeeze(gen_imgs[cnt, :,:,:]), cmap = plt.cm.bone)
                axs[i,j].axis('off')
                cnt += 1
